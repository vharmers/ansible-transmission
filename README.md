...
| -------- | ------- | ----------- |
| transmission_enable | true | Controls the running and startup state of the Transmission service |
| transmission_cli | true | Indicated if the transmission-cli package should also be installed |
...
| transmission_utp_enable | true | Enable Micro Transport Protocol |
| transmission_port_forwarding_enable | false | Enable UPnP or NAT-PMP |
...
| transmission_speed_limit_down_enable | true | Enable download speed limit |
| transmission_speed_limit_down | 5000 | The download limit in KB/s |
| transmission_speed_limit_up_enable | true | Enable upload speed limit |
| transmission_speed_limit_up | 1000 | The upload limit in KB/s |
| transmission_alt_speed_enable | false | Enable alternate speed profile |
...
| transmission_alt_speed_time_enable | false | Enable alternate speed time schedule |
...
| transmission_ratio_limit_enable | false | Enable ratio seeding limit |
| transmission_ratio_limit | 2 | Ratio seeding limit |
| transmission_download_queue_enable | true | Enable download queue |
| transmission_download_queue_size | 5 | Maximum amount of simultaneously downloaded torrents |
| transmission_seed_queue_enable | false | Enable seed queue |
| transmission_seed_queue_size | 10 | Maximum amount of simultaneously seeded torrents |
| transmission_idle_seeding_limit_enable | false | Enable idle seeding limit. This will stop the seeding of a torrent after is has been idle for a certain amount of time |
...
| transmission_queue_stalled_enable | true | When enabled, torrents that have not seen any activity for a specified amount of time, are not counted towards the seed and download queue limits |
...
| transmission_incomplete_dir_enable | false | Store files of incomplete torrents in a separate directory |
...
| transmission_rename_partial_files | true | Gives incomplete files the '.part' postfix |
| transmission_scrape_paused_torrents_enable | true | Send tracker scraping requests for paused torrents |
...
| transmission_prefetch_enable | true | Enable prefetch |
| transmission_cache_size_mb | 4 | Cache size in MB |
| transmission_watch_dir_enable | false | Watch a particular directory for new torrent files and add them |
| transmission_watch_dir | `"{{ transmission_user_home_dir }}/watch"` | Path to the directory to watch |
| transmission_pex_enable | true | Enable Peer Exchange |
| transmission_lpd_enable | false | Enable Local Peer Discovery |
...
| transmission_peer_port_random_on_start | false | Enable random peer port selection |
...
| transmission_rpc_enable | true | Enable RPC |
...
| transmission_rpc_host_whitelist_enabled | true | Enable RPC host domain whitelist |
| transmission_rpc_host_whitelist | `[]` | The domains that should have access to RPC |
| transmission_rpc_whitelist_enabled | true | Enable RPC host IP whitelist |
| transmission_rpc_whitelist | [ `"127.0.0.1"` ] | The IP's that should have access to RPC |
| transmission_rpc_authentication_required | true | Enable RPC authentication |
| transmission_rpc_username | `"transmission"` | Username to use for RPC |
| transmission_rpc_password | `""` | Password to use for RPC |
| transmission_script_torrent_done_enable | `"{{ transmission_script_torrent_done_filename | length > 0 }}"` | Execute a script if a torrent has been completed |
| transmission_script_torrent_done_filename | `""` | Path to the script |
| transmission_blocklist_enable | `"{{ transmission_blocklist_url | length > 0 }}"` | Enable the use of a blocklist |
...
| transmission_dht_enable | true | Enable Distributed Hash Table |
| transmission_start_added_torrents | true | Start newly added torrents |
| transmission_trash_original_torrent_files | false | Remove the original torrent file after it has been added |
